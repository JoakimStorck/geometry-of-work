...
def place_polar_labels_no_overlap_fast2(
    ...
    *,
    ...
    # -------------------------
    # Screen-space local basis (analytic)
    # -------------------------
    def _local_eps_r():
        return max(1e-4, 0.002 * (rmax_eff - rmin_eff + 1e-9))

    def _screen_basis_exact(theta, r):
        eps_r = _local_eps_r()
        eps_t = 1e-3
    ...
    _r_ref = float(0.5 * (rmin_eff + rmax_eff))
    _t_ref = 0.0
    (_er0_cal, _er1_cal), (_et0_cal, _et1_cal) = _screen_basis_exact(_t_ref, _r_ref)
    _scale_r = float(np.hypot(_er0_cal, _er1_cal))
    _scale_t = float(np.hypot(_et0_cal, _et1_cal)) / max(1e-9, _r_ref)

    def _screen_basis(theta, r):
        s, c = np.sin(theta), np.cos(theta)
        er0 = c * _scale_r
        er1 = s * _scale_r
        rt = max(1e-9, r) * _scale_t
        et0 = -s * rt
        et1 = c * rt
        return (er0, er1), (et0, et1)

    def _local_scales(theta, r):
        return max(1e-6, _scale_r), max(1e-6, max(1e-9, r) * _scale_t)
    ...
    for i in range(n):
        t = float(th_lab[i])
        r = float(r_lab[i])
        ha_init = "left" if np.cos(t) >= 0 else "right"
        txt = ax.text(
            t,
            r,
            labels[i],
    ...
    # =========================================================
    # ANALYTIC BBOX: measure each text's width/height ONCE,
    # then compute bbox in screen space from (x, y, ha) without
    # calling get_window_extent again.
    #
    # Reasoning:
    #   - va="center": y0 = y - h/2, y1 = y + h/2
    #   - ha="left":   x0 = x,        x1 = x + w
    #   - ha="right":  x0 = x - w,    x1 = x
    # Then apply .expanded(1.06, 1.10): around bbox center, scale w and h.
    # =========================================================
    bbs0 = [t.get_window_extent(renderer=renderer) for t in texts]
    w_px_raw = np.array([bb.width for bb in bbs0], dtype=float)   # raw widths (unexpanded)
    h_px_raw = np.array([bb.height for bb in bbs0], dtype=float)  # raw heights (unexpanded)
    # Expanded sizes (used for overlap testing — matches old .expanded(1.06, 1.10))
    _EXP_X = 1.06
    _EXP_Y = 1.10
    w_px = w_px_raw * _EXP_X
    h_px = h_px_raw * _EXP_Y

    def _make_bbox_at(x, y, ha, i):
        """Construct expanded bbox analytically from anchor (x, y) and ha."""
        w_e = w_px[i]
        h_e = h_px[i]
        # va is always "center" here
        y0 = y - 0.5 * h_e
        y1 = y + 0.5 * h_e
        if ha == "left":
            x0 = x
            x1 = x + w_e
            # When ha="left" the raw bbox is [x, x+w_raw]; expanded grows
            # around center to width w_e. Shift so center matches.
            cx = x + 0.5 * w_px_raw[i]
            x0 = cx - 0.5 * w_e
            x1 = cx + 0.5 * w_e
        else:  # "right"
    ...
    if rng is not None and init_jitter_px and init_jitter_px > 0:
        ...
        for i in range(n):
            ...
            new_ha = "left" if np.cos(t) >= 0 else "right"
            if last_ha[i] != new_ha:
                texts[i].set_ha(new_ha)
                last_ha[i] = new_ha

    # -------------------------
    # Bbox cache + dirty flag (now analytic)
    # -------------------------
    bbs = [None] * n
    ...
    def _update_one_bbox(i):
        # Analytic bbox: project (theta, r) to display, then build bbox from
        # width/height + ha. No renderer call needed.
        t = float(th_lab[i])
        r = float(r_lab[i])
        x, y = trans.transform((t, r))
        ha = last_ha[i]
        bb = _make_bbox_at(float(x), float(y), ha, i)
        bbs[i] = bb
    ...
    def _update_all_bboxes():
        # Batched transform for all dirty entries
        idx = np.flatnonzero(dirty)
        if len(idx) == 0:
            return
        pts = trans.transform(np.column_stack([th_lab[idx], r_lab[idx]]))
        for k, i in enumerate(idx):
            ha = last_ha[i]
            bb = _make_bbox_at(float(pts[k, 0]), float(pts[k, 1]), ha, i)
            bbs[i] = bb
            mid_y[i] = 0.5 * float(bb.y0 + bb.y1)
            dirty[i] = False
    ...
    # -------------------------
    # NOTE: w_px and h_px are already populated above from the initial measurement.
    r_push = rmin_eff + 0.82 * (rmax_eff - rmin_eff)
    r_band = 0.55 * (rmax_eff - rmin_eff)
    if n >= 70 or nC >= 4:
    ...
    # -------------------------
    xy_anchor = trans.transform(np.column_stack([th, rr]))
    xy_anchor = np.asarray(xy_anchor, dtype=float)
    ...
    _probe_t = theta_target
    _probe_pts0 = trans.transform(np.column_stack([_probe_t, np.full(nC, r_probe)]))
    _probe_pts1 = trans.transform(np.column_stack([_probe_t + eps, np.full(nC, r_probe)]))
    _dt = _probe_pts1 - _probe_pts0
    for c in range(nC):
        vx, vy = float(v[c, 0]), float(v[c, 1])
        if (vx * vx + vy * vy) < 1e-12:
            continue
        dtx, dty = float(_dt[c, 0]), float(_dt[c, 1])
        if (dtx * vx + dty * vy) < 0.0:
            theta_target[c] = float(theta_target[c]) + np.pi
    ...
    # Helpers
    #
    # OPTIMIZATION: _write_text no longer calls set_position/set_ha on the
    # matplotlib Text object during iteration. It only updates internal state
    # and marks dirty. The actual matplotlib write happens at the end, once
    # per label, in the "Final commit" pass. This saves a huge amount of
    # invalidation overhead (matplotlib has internal layout state).
    #
    # Because _update_one_bbox now derives bbox analytically from th_lab/r_lab
    # and last_ha[i] (not from texts[i].get_window_extent), this is safe.
    # -------------------------
    def _write_text(i):
        t = float(th_lab[i])
        new_ha = "left" if np.cos(t) >= 0 else "right"
    ...
    if any_ov:
        ...
        def _resolve_pair_best(ii, jj, need_px, y_sign_i, *, dr_clip=6.0, dth_clip=3.5):
            ...
            ha_i0, ha_j0 = last_ha[ii], last_ha[jj]
    ...
    if any_ov3:
        for _k in range(40):
            ...
            if _pair_is_near_top(ii, jj):
                ...
                _apply_x_move(ii, x_sign_i, 0.60 * need_px, dr_clip=8.0, dth_clip=6.0)
                _apply_x_move(jj, -x_sign_i, 0.60 * need_px, dr_clip=8.0, dth_clip=6.0)
    ...
    # -------------------------
    # Final clamp + COMMIT to matplotlib Text objects
    #
    # During iteration we only updated internal state (th_lab, r_lab, last_ha).
    # Now we push those final positions to the actual Text objects so the
    ...
    # Commit positions once (avoids n * MAX_ITER set_position calls during loop)
    for i in range(n):
        texts[i].set_position((float(th_lab[i]), float(r_lab[i])))
        texts[i].set_ha(last_ha[i])
